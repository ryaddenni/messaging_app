...
const SideBar = () => {
  ...
  const [selected, setSelected] = useState(); // by default 0 index button is selected
      ...
      <Stack direction="column" alignItems={"center"} justifyContent="space-between"
        ...
        <Stack alignItems={"center"} spacing={4}>
          <Box sx={{
           ...
           <img src={Logo} alt="Logo" sx={{borderRadius: 1.5}} />
          </Box>
          <Stack sx={{ width: "max-content" }} direction="column" alignItems="center" spacing={3}>
            {Nav_Buttons.map((el) => (
              el.index === selected ?
                <Box key={""} sx={{ backgroundColor: theme.palette.primary.main, borderRadius: 1.5 }}>
                  <IconButton onClick={() => { setSelected(el.index); navigate(getPath(el.index)) }} sx={{ width: "max-content", color: "#fff" }} key={el.index}>
                    {el.icon}
                  </IconButton>
                </Box>
                :
            ...
            {selected === 5 ?
              <Box sx={{ backgroundColor: theme.palette.primary.main, borderRadius: 1.5 }}>
                <IconButton onClick={() => { setSelected(5); navigate(getPath(5)) }} sx={{ width: "max-content", color: "#fff" }}>
                  <Gear />
                </IconButton>
              </Box>
              :
              <IconButton onClick={() => { setSelected(5); navigate(getPath(5)) }} sx={{
                width: "max-content",
